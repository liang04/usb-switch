...
from .theme import THEME, log_color, state_fg
...
class _Dot(QWidget):
    ...
    颜色**不缓存** —— 每次都从 :data:`THEME` 现取。缓存的代价是切主题后
    圆点还停在旧色上，而它同时被状态栏、卡片行、面板灯三处用到，
    漏一处就是「界面换肤了、几个点没换」的观感 bug。
    """
    ...
    def _paint_color(self) -> str:
        return self._color or THEME["idle"]

    def set_color(self, color: str) -> None:
        """传 ``""`` 表示「跟随主题的中性色」（即 ``THEME["idle"]``）。"""
        value = color or None
    ...
    def paintEvent(self, event) -> None:  # noqa: N802 —— Qt 命名
        ...
        painter.setBrush(QColor(self._paint_color()))
...
class StatusLight(QWidget):
    ...
    def set_state_key(self, text: str, state: str = "idle") -> None:
        ...
        self.set_state(text, state_fg(state))
...
class _CardRow(QWidget):
    ...
    def set_text(self, text: str, state: str = "idle") -> None:
        self._label.setText(text)
        # None 让圆点走 THEME["idle"] —— 它会随主题自适应，
        # 比在这里固化一个颜色多一层保险
        self._dot.set_color(state_fg(state) if state != "idle" else "")
...
class StatusBar(QWidget):
    ...
    def set_segment(self, key: str, text: str, state: str = "idle") -> None:
        ...
        if item is None:
            return
        dot, label = item
        dot.set_color(state_fg(state) if state != "idle" else "")
        label.setText(text)
...
class KeyValueGrid(QWidget):
    ...
    值的着色走 ``state`` 属性而不是色值 —— 传色值的话，切主题后这一格会
    停在旧色上（``setStyleSheet`` 是写死的）。传语义键则由 QSS 的属性
    选择器按当前主题解析。
    """
    ...
    def set_value(self, name: str, text: str, state: str | None = None) -> None:
        """``state`` 是语义键（``ok`` / ``warn`` / ``error`` / ``info``）。

        兼容旧的 ``color`` 参数：形如 ``#rrggbb`` 的字符串仍会被当作色值直用 ——
        但那条路在暗色下会失准，只留给外部还没改完的调用点。
        """
        ...
        label.setText(text)
        value = state or ""
        if value.startswith("#"):  # 旧式色值（见 docstring 的说明）
...
class LogView(QPlainTextEdit):
    ...
    def append_line(self, level: str, message: str) -> None:
        # appendHtml 的颜色是**当时**取的值，切主题后已写入的行不会自己变色。
        # 这是有意接受的：日志是历史记录，按写入时的主题着色反而更好读
        # （切到暗色后旧行若突然全变，反而像内容被改了）。
