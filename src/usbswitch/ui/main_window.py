...
from .theme import THEME, apply as theme_apply, mode_label as theme_mode_label
...
class MainWindow(QMainWindow):
    ...
    def _wire_signals(self) -> None:
        ...
        self._options.themeChanged.connect(self._on_theme_changed)
    ...
    def _on_theme_changed(self, mode: str) -> None:
        ...
        注意这里做的是「应用 + 落盘」，**不做回填** —— 下拉框是用户点的，
        它自己已经是对的状态；回填反而会多一次 ``setCurrentIndex``。

        ``auto`` 下实际生效的可能是暗色，但**存进配置的是用户的选择**
        （``auto``），不是解析结果。存解析结果的话，用户系统切一次外观、
        或换一台机器，这份配置就把人钉死在当时那个色上了。
        """
        qapp = QApplication.instance()
        applied, _palette = theme_apply(qapp, mode)
        self._config.window.theme = applied
        self._save_config()

        # 手工绘制的控件（托盘图标三态、开关、圆点）不吃 QSS，必须显式重画
        self._tray.refresh_icon()
        self._append("info", f"界面外观已切换：{theme_mode_label(applied)}")
