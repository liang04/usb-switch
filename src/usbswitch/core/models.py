...
class RemoteBridgeConfig:
    ...
    @property
    def http_base(self) -> str:
        """**展示用**地址：隧道就绪则隧道地址，否则直连地址。

        ⚠️ 不要用它发起连接 —— 它会把「隧道没通」悄悄替换成直连地址，于是
        桥接服务到底走哪条路，在连接处又多出一个判据。连接一律走
        :meth:`connect_base`。
        """
        return self.runtime_http_base or self.direct_http_base

    def connect_base(self) -> str | None:
        """实际用于**连接**桥接服务的地址；``None`` = 当前没有可用地址。

        勾了「经 SSH 隧道访问」（``use_tunnel``）就**只走隧道**：隧道未就绪时
        返回 None，**不退回直连**。

        为什么不退回直连：直连地址是远端主机自己的 ``host:bridge_port``，隧道
        没通时它几乎必然失败，而失败的形态是「连不上主机」——
        这会把用户引去排查「桥接服务有没有启动」，把真正的原因（隧道没建起来）
        盖掉。2026-09-19 就是踩在这里：日志里的真实原因是 SSH 主机密钥不符，
        界面却提示「请确认该主机上的桥接服务已启动」。宁可如实说「隧道未建立」，
        也不给一个会把诊断带偏的地址。
        """
        if self.use_tunnel:
            return self.runtime_http_base or None
        return self.direct_http_base
...
class BridgeEndpoint:
    ...
    @property
    def http_base(self) -> str:
        """展示用地址（见 :meth:`RemoteBridgeConfig.http_base` 的告警）。"""
        if self.kind is BridgeKind.LOCAL:
            return self.local.http_base
        return self.remote.http_base

    def connect_base(self) -> str | None:
        """实际用于连接的桥接地址；``None`` 表示当前没有可用地址。

        远程端勾了隧道而隧道未就绪时就是 None —— 连接方必须**如实报错**，
        不能退回 ``http_base`` 的直连兜底。
        """
        if self.kind is BridgeKind.LOCAL:
            return self.local.http_base
        return self.remote.connect_base()
...
class WindowConfig:
    ...
    #: 界面外观：``light`` / ``dark`` / ``auto``（跟随系统）。
    #: 取值合法性由 ``ui.theme.MODES`` 裁决 —— core 不 import Qt，
    #: 所以这里只存字符串，非法值在应用时收敛成 ``light``。
    theme: str = "light"
