...
class RemoteBridge:
    ...
    def base_url(self) -> str:
        """展示用地址（隧道就绪则隧道地址，否则直连地址）。"""
        return self._config.http_base

    def connect_base(self) -> str | None:
        """实际用于连接的地址；``None`` = 配置了走隧道但隧道未就绪。

        与 :attr:`base_url` 分开，是为了让「探活」与「安全弹出」用**同一个**
        判据：否则探活走直连说「在线」、弹出说「隧道未建立」，同一个分区里
        自相矛盾。
        """
        return self._config.connect_base()
    ...
    def probe(self) -> dict | None:
        ...
        隧道未就绪时**直接返回 None，不发请求** —— 直连探测给出的是另一条路的
        结论，用它点亮「桥接服务 在线」，用户会以为切换也能成。
        """
        base = self.connect_base()
        if base is None:
            log.debug("远程桥接探活跳过：配置了走隧道，但隧道尚未就绪")
            return None
        try:
            return http_json.get_json(f"{base}/status", timeout=HTTP_PROBE_TIMEOUT)
